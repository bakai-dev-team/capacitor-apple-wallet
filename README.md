...
```typescript
addPass(options: AddPassOptions) => any
```
...
| **`activationState`**            | <code><a href="#activation_state">ACTIVATION_STATE</a></code> |
...
<code>'ACTIVATED' | 'REQUIRES_ACTIVATION' | 'SUSPENDED' | 'DEACTIVATED' | 'INACTIVE'</code>
